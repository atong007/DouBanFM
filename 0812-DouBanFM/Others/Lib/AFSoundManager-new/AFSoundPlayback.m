...
-(void)listenFeedbackUpdatesWithBlock:(feedbackBlock)block andFinishedBlock:(finishedBlock)finishedBlock {
    ...
    __weak AFSoundPlayback *selfObject = self;
    _feedbackTimer = [NSTimer scheduledTimerWithTimeInterval:updateRate block:^{
        ...
        if (selfObject.statusDictionary[AFSoundStatusDuration] == selfObject.statusDictionary[AFSoundStatusTimeElapsed]) {
            
            [selfObject.feedbackTimer pauseTimer];
            
            selfObject.status = AFSoundStatusFinished;
...
-(void)remoteControlReceivedWithEvent:(UIEvent *)receivedEvent {
    WBLog(@"receivedEvent:%@", receivedEvent);
